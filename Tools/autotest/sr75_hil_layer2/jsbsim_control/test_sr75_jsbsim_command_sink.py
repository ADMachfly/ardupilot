...
class TestSR75JSBSimCommandSink(unittest.TestCase):
    ...
    def test_packet_order_matches_jsbsim_input_properties(self):
        ...
        self.assertEqual(
            command.packet_text(),
            "1.250000000,0.200000000,-0.300000000,0.400000000,0.500000000,1.000000000",
        )

    def test_command_clamps_and_rejects_bad_values(self):
        command = JSBSimActuatorCommand(1.0, 2.0, -2.0, 0.0, 1.5, -0.5).validated()
    ...
    def test_udp_send_and_stale_once(self):
        ...
        try:
            ...
            self.assertEqual(
                data.decode("ascii"),
                "2.000000000,0.100000000,0.200000000,-0.300000000,0.400000000,0.000000000",
            )

            self.assertIsNotNone(sink.send_stale_once(timestamp_s=3.0))
            data, _ = server.recvfrom(2048)
            self.assertEqual(
                data.decode("ascii"),
                "3.000000000,0.000000000,0.000000000,0.000000000,0.000000000,0.000000000",
            )
