#!/usr/bin/env python3
# AP_FLAKE8_CLEAN
"""F22-GZ-AE: isolate ArduPlane-owned RATO eject after resumed BOOST.

F22-GZ-AD proved resume_boost()/BOOST->BURNOUT work, but handing FULL
control to ArduPlane's own live AUTO/TECS attitude control (the normal
--auto-gate-hold release) caused a severe, unrecoverable dive before
ENGINE_TAKEOVER->EJECT could be reached -- a flight-control-law problem,
not a defect in the new resume API, and explicitly out of scope to fix
(no gain/mission/TECS tuning here).

This harness instead keeps the airframe stable using the SAME proven
responder precontrol schedule as F22-GZ-AA (elevator=+0.10/throttle=0.37
during the resumed burn, elevator=0.00/throttle=0.37 after, via
--precontrol-burn-duration-s), while ArduPlane still independently arms,
goes AUTO, and calls resume_boost() -- and, new for this task, the
responder's --auto-gate-hold override now optionally passes the LIVE
decoded rato/eject fields through (--auto-gate-live-rato-eject) instead of
also fixing those to the precontrol reference. So elevator/throttle/
airframe attitude are harness-stabilized, but RATO ignition/burn/eject
timing is entirely owned by ArduPlane's RATOController, driven through
RATO_IGN_CH/RATO_EJ_CH exactly as on real hardware. The auto-gate release
file is intentionally never created, so this hybrid mode holds for the
entire run -- no dual command writers, no responder restart, no JSBSim
runscript eject override.
"""
import csv
import math
import os
...
RUN = Path(os.environ.get("SR75_D3_RUN", "/tmp/sr75_f22gzae_resume"))
