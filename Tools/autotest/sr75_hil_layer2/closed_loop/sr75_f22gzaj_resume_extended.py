#!/usr/bin/env python3
# AP_FLAKE8_CLEAN
"""F22-GZ-AJ: extend preserved-state live-control RATO handoff to a
longer AUTO climb.

F22-GZ-AI validated live ArduPlane AUTO/TECS control (CH1-CH4 all live
from the first dynamic frame, no fixed precontrol reference anywhere)
from the exact preserved release IC (theta=20deg, ubody=43.85 m/s, zero
elapsed decay via the delayed-JSBSim-launch ordering from F22-GZ-AH) over
a 30s window: full RATO resume -> BOOST -> BURNOUT -> ENGINE_TAKEOVER ->
EJECT -> COMPLETE, dry booster ejects 10kg->0kg once, engine symmetry
exact, and a genuinely climbing (not diving) trajectory to ~+601m by t=30.

This harness forks AI unchanged except for the run duration: the
...
exists) and the fully-live CH1-CH4 control are kept identical. The
monitoring window and the JSBSim runscript's <run end=...> are both
extended from 30s to 120s, to observe whether the post-EJECT/
ENGINE_TAKEOVER aircraft (mission: NAV_TAKEOFF then a distant
NAV_WAYPOINT) stays bounded and begins transitioning from the initial
launch climb toward ordinary AUTO waypoint-following behavior, rather
than only ever being validated over the short RATO-sequence window.
Diagnostic only: no aero/gain/mission/TECS/RATOController tuning.
"""
import csv
import math
import os
...
RUN = Path(os.environ.get("SR75_D3_RUN", "/tmp/sr75_f22gzaj_resume"))
...
<runscript xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <run start="0.0" end="130.0" dt="0.008333">
...
def main():
    ...
    try:
        ...
        # F22-GZ-AJ: extended from 30s to 120s to observe whether the
        # post-EJECT aircraft stays bounded and begins transitioning toward
        # ordinary AUTO waypoint-following behavior.
        end = time.time() + 120
        modes_seen = []
        failsafe_lines = []
        while time.time() < end:
            ...
            if msg is not None and msg.get_type() == "STATUSTEXT":
                ...
                if "failsafe" in msg.text.lower() or "crash" in msg.text.lower():
                    failsafe_lines.append((time.time() - (end - 120), msg.text))
            if msg is not None and msg.get_type() == "HEARTBEAT" and msg.get_srcComponent() == hb_comp:
                ...
                if not modes_seen or modes_seen[-1][1] != mode:
                    modes_seen.append((time.time() - (end - 120), mode))
        ...
        for row in rows:
            ...
            prev_e2 = e2
            prev_dry = dry
            for snap_t in (5.0, 10.0, 15.0, 20.0, 30.0, 60.0, 90.0, 120.0):
                key = f"t{int(snap_t)}"
