#!/usr/bin/env python3
# AP_FLAKE8_CLEAN
"""F22-GZ-AI: validate live ArduPlane control using preserved-state RATO
resume handoff.

F22-GZ-AH validated that JSBSim can be launched fresh, exactly when
ArduPlane is armed and AUTO-ready, so the release IC (theta=20deg,
ubody=43.85 m/s) is preserved with zero elapsed decay -- but AH still kept
elevator/aileron/rudder/throttle pinned to the fixed, harness-proven
precontrol reference the whole time (via --auto-gate-hold
--auto-gate-live-rato-eject), only letting RATO ignition/eject be live.

This harness forks AH and keeps the exact same preserved-state launch
ordering (responder+ArduPlane ready first, JSBSim launched last, state
force-released to ArduPlane only once JSBSim's first real row exists --
see sr75_sim_json_responder.py's B3StartupSync.force_release()), but
removes --auto-gate-hold/--auto-gate-live-rato-eject entirely. Without
that override, B3PrecontrolHandover's own existing self-release rule
applies instead: once ArduPlane produces its first valid CH1-CH4 PWM frame
(which happens early during boot, harmlessly, since JSBSim isn't running
yet), it hands over to raw ArduPlane PWM permanently. So by the time
JSBSim actually launches and the state force-releases, ArduPlane's own
live PWM has ALREADY been the sole actuator command source the whole
time -- elevator/aileron/rudder/throttle AND RATO/eject are all live
ArduPlane control from the very first dynamic frame, with no fixed
reference anywhere in the loop. --precontrol-burn-duration-s/postburn-*
are dropped entirely (nothing to switch between). This is diagnostic
only: no aero/gain/mission/TECS/RATOController tuning, just observing
what live control does and where it first departs from AH's fixed
reference (elevator=+0.10/throttle=0.37 during burn,
elevator=0.00/throttle=0.37 after).
"""
import csv
import math
import os
...
RUN = Path(os.environ.get("SR75_D3_RUN", "/tmp/sr75_f22gzai_resume"))
...
CASE = "f22gzai_resume_livecontrol"
JSB_PORT = 5610
SIM_JSON_PORT = 9002
MAVLINK_PORT = 5780
HOME_LAT = 32.5378147
