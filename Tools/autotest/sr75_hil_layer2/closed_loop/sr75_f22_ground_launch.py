#!/usr/bin/env python3
# AP_FLAKE8_CLEAN
"""SR-75 Layer 2D-F22 true ground-launch RATO test.

Forked from sr75_f21_auto_takeoff.py (F21-L, PASS). F21 starts from the
flying layer2j_b3_cruise_trim_init IC (TAS=69 m/s) -- F22 instead starts
from f22_ground_launch_init.xml, a true zero-speed launcher IC (TAS=0,
site altitude, rail pitch, mission yaw), to see how RATO ignition/burn/
handoff/ejection behave from an actual zero-speed ground launch rather than
an already-flying state.

Still drives JSBSim + sr75_sim_json_responder.py + a real ArduPlane SITL
binary through mission upload, arm, AUTO, RATO ignition/burn/handoff/
ejection, and prints a diagnostic summary. Still uses the responder's
--auto-gate-hold so the fixed precontrol reference is the only command
JSBSim ever sees until MODE=AUTO is confirmed -- this keeps ArduPlane's
default FBWA boot mode from ever driving the FDM before RATO can legally
ignite (RATOController.update() is itself gated on AUTO+TAKEOFF, see
ArduPlane/commands_logic.cpp). Note: RATOController::State::READY itself
additionally requires speed_mps>=1 or dist_m>=1 before it will transition to
IGNITION (see ArduPlane/rato.cpp) -- with precontrol turbojet throttle held
at 0 during the pre-AUTO gate window, this means RATO cannot ignite the
instant AUTO engages from a true zero-speed start; some turbojet-only
ground roll after AUTO (once ArduPlane's own throttle command takes over)
is expected and is not suppressed here, since fixing that would require
editing ArduPlane/rato.cpp, out of scope for this harness-only test.
"""
import csv
import math
import os
...
RUN = Path(os.environ.get("SR75_D3_RUN", "/tmp/sr75_layer2df22_A"))
...
# Pre-release precontrol elevator. Aerodynamically inert at the F22 ground
# IC's zero airspeed (no dynamic pressure); kept at the F21-H sweep value
# for continuity/simplicity rather than left arbitrary.
PRECONTROL_ELEVATOR = -0.49
# F22: turbojet throttle held at idle (not F21's 0.37) during the pre-AUTO
# precontrol/gate window, so the aircraft cannot taxi/roll under its own
# thrust before AUTO is confirmed ("no pre-ignition motion").
PRECONTROL_THROTTLE = 0.0
CASE = "rato_ground_launch"
JSB_PORT = 5601
# Must equal ArduPlane's default --sim-port-out (SIM_OUT_PORT=9002 in
# SITL_cmdline.cpp); the JSON responder replies via sendto(payload, source)
# on a single bidirectional socket, so it must listen on the same port
# ArduPlane sends its servo/control packets from. A prior value of 9003 here
# (an attempt at port isolation from F21) broke this pairing and caused
# ArduPlane to loop on "No JSON sensor message received" forever.
SIM_JSON_PORT = 9002
MAVLINK_PORT = 5771
...
def stop_stale():
    ...
    for pattern in (
        ...
        "JSBSim.*sr75_f22_ground_launch.xml",
...
def write_files():
    ...
    runscript = RUN / "sr75_f22_ground_launch.xml"
...
<runscript xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    name="SR75 Layer 2D-F22 ground-launch RATO closed-loop input">

  <use aircraft="sr_75_6_dof" initialize="f22_ground_launch_init"/>
...
def main():
    ...
    try:
        ...
        resp_cmd = [
            ...
            "--startup-sync-airspeed-mps", "0.0", "--startup-sync-latitude-deg", "32.5378085",
