#!/usr/bin/env python3
# AP_FLAKE8_CLEAN
"""F22-GZ-AK: validate preserved-state RATO handoff into full AUTO
waypoint mission continuation.

F22-GZ-AJ validated 120s of live ArduPlane AUTO/TECS control (CH1-CH4 all
live from the first dynamic frame) from the exact preserved release IC
(theta=20deg, ubody=43.85 m/s, zero elapsed decay via the delayed-JSBSim-
launch ordering from F22-GZ-AH): full RATO resume -> BOOST -> BURNOUT ->
ENGINE_TAKEOVER -> EJECT -> COMPLETE, dry booster stays 0kg after eject,
engine symmetry exact, AUTO mode never left, no failsafe/crash, and a
sustained, controlled climb (not diving) to ~+1685m by t=120.

This harness forks AJ unchanged except for the run duration: the
preserved-state launch ordering (responder+ArduPlane ready first, JSBSim
launched last, state force-released only once JSBSim's first real row
exists) and the fully-live CH1-CH4 control are kept identical, and the
SAME mission file already used throughout this task series
(SR75_LAYER2D_F21_AUTO_TAKEOFF.waypoints: NAV_TAKEOFF then a single
distant NAV_WAYPOINT ~12km away) is kept -- this already is the real
F22/AUTO mission file, not a synthetic short climb; no other/richer SR75
mission file exists in this repo (confirmed by search), so there is
nothing to switch to. The monitoring window and the JSBSim runscript's
<run end=...> are both extended from 120s to 300s, and MISSION_CURRENT
(seq) is now explicitly requested and tracked over the whole run to
directly verify waypoint-index progression alongside the existing
trajectory bounds/failsafe/eject/symmetry checks. Diagnostic only: no
aero/gain/mission/TECS/RATOController tuning.
"""
import csv
import math
import os
...
RUN = Path(os.environ.get("SR75_D3_RUN", "/tmp/sr75_f22gzak_resume"))
...
<runscript xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <run start="0.0" end="310.0" dt="0.008333">
...
def main():
    ...
    try:
        ...
        for row in rows:
            ...
            prev_e2 = e2
            prev_dry = dry
            for snap_t in (5.0, 10.0, 15.0, 20.0, 30.0, 60.0, 90.0, 120.0, 150.0, 180.0, 210.0, 240.0, 270.0, 300.0):
                key = f"t{int(snap_t)}"
        ...
        for key in ("t5", "t10", "t15", "t20", "t30", "t60", "t90", "t120", "t150", "t180", "t210", "t240", "t270", "t300"):
            if key in snapshots:
                th, alt, tas, e0n, e1n = snapshots[key]
                print(f"{key:>4} {th:8.2f} {alt:9.2f} {tas:8.2f} {e0n:9.1f} {e1n:9.1f}")

    except Exception as e:
        print(f"F22GZAK_RUN_ERROR: {e}", file=sys.stderr)
