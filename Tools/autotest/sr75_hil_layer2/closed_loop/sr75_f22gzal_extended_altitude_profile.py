#!/usr/bin/env python3
# AP_FLAKE8_CLEAN
"""F22-GZ-AL: create and validate an extended SR-75 AUTO mission using an
altitude-phase profile (climb/cruise/descend/loiter/return/final-descent).

F22-GZ-AK validated the full preserved-state handoff into a complete
short AUTO mission (RATO resume -> BOOST -> BURNOUT -> ENGINE_TAKEOVER ->
EJECT -> COMPLETE, dry booster stays 0kg after eject, engine symmetry
exact, waypoint index advances, mission complete -> auto RTL, no
failsafe/crash) over ~140s using the original 2-waypoint
SR75_LAYER2D_F21_AUTO_TAKEOFF.waypoints file.

This harness forks AK unchanged except for the mission: a NEW file,
SR75_F22_EXTENDED_ALTITUDE_PROFILE.waypoints, adds an 8-phase altitude
profile after the identical (byte-for-byte copied) home+NAV_TAKEOFF
items that already carry the validated RATO/takeoff handoff -- climb to
5000m, cruise outbound at 5000m, descend to 1000m, loiter near 1000m
(500m radius, 90s), ascend back to 5000m, cruise return at 5000m, final
descent to 500m, and a final near-home waypoint at 500m whose completion
triggers ArduPlane's own auto-RTL (as already validated in AK) -- ground
landing is explicitly excluded. All legs run along a single 315deg
outbound/return bearing (matching the established launch heading) with
generous distance (25-30km per climb/descent leg, ~10-24% grades) so
TECS has room to manage altitude changes without any gain/TECS tuning.
...
exists) and fully-live CH1-CH4 control are unchanged from AI/AJ/AK. The
monitoring window and the JSBSim runscript's <run end=...> are extended
to cover the full ~150-160km round-trip mission. Diagnostic only: no
aero/gain/TECS/RATOController/engine-mapping/responder-protocol edits.
"""
import csv
import math
import os
...
RUN = Path(os.environ.get("SR75_D3_RUN", "/tmp/sr75_f22gzal_extended"))
MISSION_FILE = REPO / "Tools/autotest/sr75_hil_layer2/closed_loop/SR75_F22_EXTENDED_ALTITUDE_PROFILE.waypoints"
FORCE_RELEASE_FILE = None      # set in main() to RUN / "startup_sync_force_release.flag"
IC_NAME = "f22gzo_release_state_init"
CASE = "f22gzal_extended_altitude_profile"
JSB_PORT = 5613
SIM_JSON_PORT = 9002
MAVLINK_PORT = 5783
HOME_LAT = 32.5378147
...
<runscript xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <run start="0.0" end="1830.0" dt="0.008333">
...
def main():
    ...
    try:
        ...
        for row in rows:
            ...
            prev_e2 = e2
            prev_dry = dry
            for snap_t in (5.0, 10.0, 15.0, 20.0, 30.0, 60.0, 120.0, 180.0, 240.0, 300.0, 360.0, 420.0, 480.0, 540.0, 600.0, 660.0, 720.0, 780.0, 840.0, 900.0, 960.0, 1020.0, 1080.0, 1140.0, 1200.0, 1260.0, 1320.0, 1380.0, 1440.0, 1500.0, 1560.0, 1620.0, 1680.0, 1740.0, 1800.0):
                key = f"t{int(snap_t)}"
        ...
        for key in ("t5", "t10", "t15", "t20", "t30", "t60", "t120", "t180", "t240", "t300", "t360", "t420", "t480", "t540", "t600", "t660", "t720", "t780", "t840", "t900", "t960", "t1020", "t1080", "t1140", "t1200", "t1260", "t1320", "t1380", "t1440", "t1500", "t1560", "t1620", "t1680", "t1740", "t1800"):
            if key in snapshots:
                th, alt, tas, e0n, e1n = snapshots[key]
                print(f"{key:>4} {th:8.2f} {alt:9.2f} {tas:8.2f} {e0n:9.1f} {e1n:9.1f}")

    except Exception as e:
        print(f"F22GZAL_RUN_ERROR: {e}", file=sys.stderr)
